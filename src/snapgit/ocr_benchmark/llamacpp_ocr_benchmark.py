...
def _ensure_output_dir(path: Path | None, model: str) -> Path:
    if path is not None:
        ...
        output_dir = Path("data/ocr_bench/llamacpp") / safe_model / now
...
def run_benchmark(
    *,
    ...
    for image_path in image_paths:
        ...
        try:
            raw_text, backend_payload = _run_llamacpp_ocr(
    ...
    markdown_summary = [
        "# llama.cpp OCR Benchmark Summary",
        "",
        f"- Model: `{model}`",
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run OCR benchmark for llama.cpp OpenAI-compatible server and save artifacts."
    )
    parser.add_argument(
        "--input-glob",
        default="/tmp/scr/*.png",
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Output directory. Default: data/ocr_bench/llamacpp/<model>/<timestamp>.",
    )
    parser.add_argument(
        "--api-base-url",
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Model alias/name visible in /v1/models.",
    )
    parser.add_argument(
        "--language-hint",
        default="pl,en",
